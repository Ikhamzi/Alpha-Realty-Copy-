...
const STYLES = `
  ...
  .login-page::before {
    ...
    background: radial-gradient(ellipse at 70% 20%, #ECE4FC55 0%, transparent 55%), radial-gradient(ellipse at 15% 80%, #D4EEF230 0%, transparent 50%);
  }
  ...
  .input-wrap svg { position: absolute; left: 12px; top: 50%; transform: translateY(-50%); color: #C4B8E8; }
  ...
  .field-input:focus { border-color: #7C6FAB; background: #fff; }
  .field-input::placeholder { color: #C4B8E8; }

  .btn-submit { width: 100%; padding: 12px; font-family: 'DM Sans', sans-serif; font-size: 11px; font-weight: 600; letter-spacing: 0.12em; text-transform: uppercase; background: #7C6FAB; color: #FAF7F2; border: none; border-radius: 2px; cursor: pointer; transition: all 0.25s; }
  .btn-submit:hover { background: #6B5F99; transform: translateY(-1px); box-shadow: 0 6px 20px rgba(124,111,171,0.3); }
...
const Wolf = ({ size = 36 }) => (
    <svg width={size} height={size * 1.1} viewBox="0 0 100 110" fill="none">
        <defs>
            <linearGradient id="la" x1="0" y1="0" x2="1" y2="1"><stop stopColor="#7C6FAB" /><stop offset="1" stopColor="#B8ADDC" stopOpacity=".9" /></linearGradient>
            <linearGradient id="lb" x1="1" y1="0" x2="0" y2="1"><stop stopColor="#5B8FA8" stopOpacity=".9" /><stop offset="1" stopColor="#9BBFCC" stopOpacity=".8" /></linearGradient>
        </defs>
        <polygon points="50,4 94,26 94,74 50,106 6,74 6,26" fill="none" stroke="#C4B8E8" strokeWidth="1.2" />
        <polygon points="50,10 72,30 50,24" fill="url(#la)" /><polygon points="50,10 28,30 50,24" fill="url(#lb)" />
    ...
    return (
        <>
            ...
            <div className="login-page">
                ...
                {/* Decorative rings */}
                {[[220, 220, '8%', '4%', '#C4B8E818'], [140, 140, '14%', '9%', '#C4B8E828']].map(([w, h, t, l, b], i) => (
